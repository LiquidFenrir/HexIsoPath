...
void drawGrid(HexPiece * grid, Team * teams, TeamsColor team, int selectedToken, HexPieceSides selectedSide, HexPiece * sourceHex, HexPiece * destinationHex)
{
	...
	for (; offset < homeRowSize-1; offset++) {
		pp2d_draw_texture_part_blend(TEXTURE_HEX, grid[offset].xPos+16, grid[offset].yPos-8, 0, 16, 32, 16, COLOR_WHITE);
		drawHex(grid[offset]);
	}
	drawHex(grid[offset]);
	offset++;
	...
	for (; offset < gridCompleteSize-1; offset++) {
		pp2d_draw_texture_part_blend(TEXTURE_HEX, grid[offset].xPos+16, grid[offset].yPos+24, 0, 0, 32, 16, COLOR_BLACK);
		drawHex(grid[offset]);
	}
	drawHex(grid[offset]);
