...
static void pp2d_blend(void);
static void pp2d_draw_immediate(size_t id, int x, int y, int height, int width, float left, float right, float top, float bottom);
static void pp2d_draw_vbo(size_t id, int x, int y, int height, int width, float left, float right, float top, float bottom);
static void pp2d_fast_draw_vbo(int x, int y, int height, int width, float left, float right, float top, float bottom);
...
static void (*pp2d_draw_texture_internal)(size_t, int, int, int, int, float, float, float, float);
...
void pp2d_draw_text_center(gfxScreen_t target, int y, float scaleX, float scaleY, u32 color, const char* text)
{
	...
	int x = ((target == GFX_TOP ? TOP_WIDTH : BOTTOM_WIDTH) - (int)width) / 2;
...
Result pp2d_init(void)
{
	...
	pp2d_set_texture_drawing_method(VBO);
...
{
	return pp2d_draw_texture_internal == pp2d_draw_vbo ? VBO : IMMEDIATE;
}
...
void pp2d_set_texture_drawing_method(drawingMethod method)
{
	if (method == VBO)
	{
		pp2d_draw_texture_internal = pp2d_draw_vbo;
	}
	else if (method == IMMEDIATE)
	{
		pp2d_draw_texture_internal = pp2d_draw_immediate;
	}
}
